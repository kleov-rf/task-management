...
describe('LocalStorageTaskRepository', () => {
  it('should call localStorage when saving a task', async () => {
    ...
    const expectedTask = JSON.stringify(
      Array.from(new Map().set(mockTaskId, mockTask.toPrimitives()).entries())
    )
  ...
  it('should call localStorage with new and existing tasks when saving a task', async () => {
    ...
    const existingTasks = new Map().set(
      mockExistingTaskId,
      Task.create({
    ...
    const localStorageMock = {
      ...
      getItem: vi
        .fn()
        .mockReturnValue(JSON.stringify(Array.from(existingTasks.entries())))
    }
    Object.defineProperty(window, 'localStorage', { value: localStorageMock })
    const mockDueDate = new Date().getTime()
    const mockTaskId = '2'
    ...
    const localStorageMock = {
      getItem: vi
        ...
        .mockReturnValue(
          JSON.stringify(
            Array.from(
              new Map().set(mockTaskId, mockTask.toPrimitives()).entries()
            )
          )
        )
    }
